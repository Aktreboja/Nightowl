import { RecommendationQuery, SpotifyResponseError } from "../../../../spotify_api";
...
export const GetRecommendations = async (access_token: string, query: RecommendationQuery) : Promise<RecommendationsResponse> => {
    try {
        ...
        const recommendationResponse = await fetch(process.env.NEXT_PUBLIC_SPOTIFY_API_BASE + `/recommendations?${recArray}&market=ES&limit=40`, {
            method: 'GET',
            headers: {'Authorization': "Bearer " + access_token}
        })
        if (recommendationResponse.ok) return await recommendationResponse.json() as RecommendationsResponse;
        else {
            const errorData : SpotifyResponseError = await recommendationResponse.json();
            throw new Error(`Error retrieving recommendations: ${recommendationResponse.status} - ${errorData.error.message}`)
        }
    } catch (error) {
        throw new Error(`Unable to fetch Recommendations: ${(error as Error).message}`)
    }
