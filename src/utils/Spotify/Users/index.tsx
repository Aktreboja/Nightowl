 ...
 */
export const getUserData = async (access_token: string) : Promise<User | SpotifyResponseError> => {
    try {
        const response = await fetch(process.env.NEXT_PUBLIC_SPOTIFY_API_BASE + "/me", {
            method: "GET",
        ...
        if (response.ok) {
            ...
            throw new Error(`Failed to fetch user data: ${response.status} - ${errorData.error.message}`)
        }
    } catch (error) {
