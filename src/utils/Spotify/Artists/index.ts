...
export const GetTopArtists = async (access_token: string, time_range : string) : Promise<TopItems>  => {
    try {
        ...
        if (response.ok) return await response.json() as TopItems;
            ...
            throw new Error(`Unable to fetch Artist: ${response.status} - ${errorData.error.message}`)
        }
    } catch (error) {
 ...
 */
export const GetArtist = async (access_token: string, id: string) : Promise< Artist | SpotifyResponseError> => {
    try {
        const response = await fetch(process.env.NEXT_PUBLIC_SPOTIFY_API_BASE + `/artists/${id}`, {
            method: "GET",
            headers: {'Authorization': "Bearer " + access_token}
        })
 ...
 */
export const GetSeveralArtists = async (access_token: string, ids: string[]) : Promise<Artist[] | SpotifyResponseError>=> {
    try {
        const idsQuery = (ids.map((id) => id)).join(',');
        const response = await fetch(process.env.NEXT_PUBLIC_SPOTIFY_API_BASE + `/artists?ids=${idsQuery}`, {
            method: "GET",
            headers: {'Authorization': "Bearer " + access_token}
        })
        if (response.ok) {
            ...
            const errorData: SpotifyResponseError = await response.json();
            throw new Error(`Error fetching several artists: ${response.status} - ${errorData.error.message}`)
        }
    } catch (error) {
        throw new Error(`Unable to get Several Artists: ${(error as Error).message}`)
    }
}
 ...
 */
export const GetArtistsTopTracks = async (access_token: string, id: string) : Promise<Track[] | SpotifyResponseError> => {
    try {
        const response = await fetch(process.env.NEXT_PUBLIC_SPOTIFY_API_BASE + `/artists/${id}/top-tracks?market=ES`, {
            method: "GET",
            headers: {'Authorization': "Bearer " + access_token}
        })
        if (response.ok) return await response.json() as Track[];
            ...
            const errorData: SpotifyResponseError = await response.json();
            throw new Error(`Error fetching Artist's top tracks: ${response.status} - ${errorData.error.message}`)
        }
    } catch (error) {
        throw new Error(`Unable to get Artist's top tracks: ${(error as Error).message}`)
    }
}
 ...
 */
export const GetArtistsAlbums = async (access_token: string, id: string) : Promise<Page<SimplifiedAlbum> | SpotifyResponseError>=> {
    try {
        const response = await fetch(process.env.NEXT_PUBLIC_SPOTIFY_API_BASE + `/artists/${id}/albums`, {
            method: "GET",
            headers: {'Authorization': "Bearer " + access_token}
        })
        if (response.ok) return await response.json() as Page<SimplifiedAlbum>
            ...
            const errorData: SpotifyResponseError = await response.json();
            throw new Error(`Error fetching Artist's albums: ${response.status} - ${errorData.error.message}`)
        }
    } catch (error) {
        throw new Error(`Unable to get Artist's albums: ${(error as Error).message}`)
    }
}
 ...
 */
export const GetRelatedArtists = async (access_token: string, id: string) : Promise<Artist[] | SpotifyResponseError>=> {
    try {
        const response = await fetch(process.env.NEXT_PUBLIC_SPOTIFY_API_BASE + `/artists/${id}/related-artists`, {
            method: "GET",
            headers: {'Authorization': "Bearer " + access_token}
        })
        if (response.ok) return await response.json() as Artist[];
            ...
            const errorData: SpotifyResponseError = await response.json();
            throw new Error(`Error fetching related artists: ${response.status} - ${errorData.error.message}`)
        }
    } catch (error) {
