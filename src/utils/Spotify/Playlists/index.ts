...
export const GetUserPlaylists = async (
  ...
  try {
    const response = await fetch(
      process.env.NEXT_PUBLIC_SPOTIFY_API_BASE + '/me/playlists',
      {
        method: 'GET',
...
export const CreatePlaylist = async ({
  ...
  const response = await fetch(
    process.env.NEXT_PUBLIC_SPOTIFY_API_BASE + `/users/${userId}/playlists`,
    {
      method: 'POST',
      headers: { Authorization: `Bearer ${access_token}` },
      body: new URLSearchParams(playlistData),
    },
  );
};
