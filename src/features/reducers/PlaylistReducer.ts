...
const PlaylistReducer = createSlice({
    ...
    reducers: {
        ...
        addTrackToQueue: (state, action: PayloadAction<Track>) => {
            const existingTrack = state.queue.find(track => track.id === action.payload.id)
            if (existingTrack) {
        ...
        removeTrackFromQueue: (state, action: PayloadAction<Track>) => {
            ...
            const removedIndex = state.queue.findIndex(track => track.id === action.payload.id)
    ...
    extraReducers(builder) {
        builder.addCase(getUserPlaylists.fulfilled, (state, action: PayloadAction<Playlist[]>) => {
...
export const { addTrackToQueue, clearTrackQueue, removeTrackFromQueue, updatePlaylistDescription, updatePlaylistName } = PlaylistReducer.actions;
